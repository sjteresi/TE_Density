...
from preprocess import split, check_groupings
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Parent directory to output temporary overlap data",
    )
    parser.add_argument(
        "--filtered_input_data",
        "-f",
        ...
        help="parent directory for filtered input data",
    )
    # NOTE
    ...
    parser.add_argument(
        "--reset_h5",
        action="store_true",
        help="""When flag is provided resets the cache of
                        wrapped GeneData and TransposonData input files.""",
    )
    parser.add_argument(
        "--contig_del",
        action="store_false",
        help="""When flag is provided will not delete any
                        entries from input gene or TE annotation that have an
                        item in the chromosome field with 'Contig'.""",
    )
    parser.add_argument(
        "--reset_revise_anno",
        action="store_true",
        help="""When flag is provided always re-writes the revised
                       annotation file.""",
    )
    parser.add_argument(
        "--dont_use_revise_anno",
        action="store_true",
        help="""When flag is provided does not use a revised
                        annotation file at all, does not create a revised
                        annotation at all.""",
    )

    # TODO make this less dependent on the filtered_input_data arg
    # Ask Michael, I am not sure the best way to do that.
    parser.add_argument(
        "--revised_input_data",
        "-r",
        type=str,
    ...
    te_data_unwrapped = revise_annotation(
        te_data_unwrapped,
        args.reset_revise_anno,
        args.dont_use_revise_anno,
        revised_transposons,
