...
class Normalize:
    """Functions for calculating normalized values

    Calculate the divisor matrix necessary for TE density calculation.

    TE Density is the calculation of (TE base-pairs / relevant area), where
    relevant area is the window value for the right-hand side of genes, the gene
    length for intronic calculations, and a clipped window value for the
    left-hand side of genes in case the window starts to extend past 0 becoming
    negative, which would not make biological sense. Genome BP position bounds
    are [0, inf).
    """

    @unique
    class Direction(Enum):
        """Where the normalization is calculated relevant to the gene."""

        # NOTE I don't think this part is used
    ...
    def divisor_left(GeneData, windows):
        """Divisors to the left of every gene for every window.
        ...
        Returns:
            divisor_dataframe (numpy array): Numpy array of shape
                (n_windows x n_genes). Dataframe of values represents the "relevant
                area" used in density calculations for a single chromosome.

        Example:
            200 BP Gene Start with a 500 BP window moving to the left, the
            window cannot go negative past 0, so it is clipped to 0, and
            relevant area calculated to be 200
        """
        divisor_dataframe = pd.DataFrame(
        ...
        for window in windows:
            for name in GeneData.names:
                ...
                divisor_dataframe.at[window, name] = win_length  # NOTE
                # this step is a little slow and there is likely an optimal
                # speed solution
        return divisor_dataframe.to_numpy(copy=False)

    @staticmethod
    def divisor_intra(GeneData, windows):
        """Divisors for the intronic area of every gene for every window

        Intronic calculations: Gene length, here we are considering intronic
        TEs, so the relevant area is the gene length. Data is still formatted
        for each window to match foratting of left and right, but the value
        will not change between windows (gene length stays the same).
    ...
    def divisor_right(GeneData, windows):
        """Divisors to the right of every gene for every window.
        ...
        for window in windows:
            for name in GeneData.names:
                gene_datum = GeneData.get_gene(name)
                win_length = gene_datum.win_length(window)
                divisor_dataframe.at[window, name] = win_length
        return divisor_dataframe.to_numpy(copy=False)

    def __repr__(self):  # TODO SCOTT did you mean to use __str__? __repr__ is for eval style output
        """
