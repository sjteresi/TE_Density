...
class GenomeData(object):
    ...
    def __init__(self, genome_id, gene_dataframe, transposon_dataframe,
        ...
        if self.genome_size != None:
            self.total_other_lengths_MB = self.genome_size - self.total_G_lengths_MB - self.total_T_lengths_MB
    ...
    def Chromosomes(self):
        ...
        """
        comparison = self.gene_dataframe.Chromosome.unique() == \
        self.transposon_dataframe.Chromosome.unique()

        if comparison.all():
            return self.gene_dataframe.Chromosome.unique().tolist()
        else:
            raise ValueError('''The chromosomes in the transposon dataframe do
    ...
    def order_sum_seq_len_dict_MB(self):
        return self.transposon_dataframe.groupby(['Order']).Length.sum().divide(1000000000).to_dict()
...
class SubgenomeData(GenomeData):
    ...
    def __init__(self, genome_id, gene_dataframe, transposon_dataframe,
        ...
        """
        self.gene_dataframe = gene_dataframe.loc[gene_dataframe['Chromosome'].isin(chromosome_grouping)]
        self.transposon_dataframe = transposon_dataframe.loc[transposon_dataframe['Chromosome'].isin(chromosome_grouping)]
        self.genome_id = genome_id
    ...
    def get_subgenome_genome_size(self, gene_dataframe, transposon_dataframe,
                                  chromosome_grouping):
